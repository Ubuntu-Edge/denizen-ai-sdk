...
class DenizenToolSession extends DenizenSession {
  ...
  @override
  Future<String> chat(String prompt, {int maxTurns = 5}) {
    return sessionQueue.run(() async {
      ...
      while (turnCount < maxTurns) {
        // 1. Generate text response using unqueued internal turn since we are inside sessionQueue
        latestResponse = await chatInternal(currentPrompt);
  ...
  @override
  Stream<String> streamChat(String prompt, {int maxTurns = 5}) {
    return sessionQueue.runStream(() async* {
      ...
      while (turnCount < maxTurns) {
        ...
        final stream = streamChatInternal(currentPrompt);
